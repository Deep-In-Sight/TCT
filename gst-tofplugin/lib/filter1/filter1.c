...
static void gst_filter1_set_property(GObject *object, guint property_id,
                                     const GValue *value, GParamSpec *pspec);
static void gst_filter1_get_property(GObject *object, guint property_id,
...
static gboolean gst_filter1_accept_caps(GstBaseTransform *trans,
                                        GstPadDirection direction,
                                        GstCaps *caps);
static gboolean gst_filter1_set_caps(GstBaseTransform *trans, GstCaps *incaps,
                                     GstCaps *outcaps);
...
static gboolean gst_filter1_sink_event(GstBaseTransform *trans,
                                       GstEvent *event);
static gboolean gst_filter1_src_event(GstBaseTransform *trans, GstEvent *event);
static GstFlowReturn gst_filter1_prepare_output_buffer(GstBaseTransform *trans,
                                                       GstBuffer *input,
                                                       GstBuffer **outbuf);
...
static GstStaticPadTemplate gst_filter1_src_template = GST_STATIC_PAD_TEMPLATE(
    "src", GST_PAD_SRC, GST_PAD_ALWAYS, GST_STATIC_CAPS("application/unknown"));
...
G_DEFINE_TYPE_WITH_CODE(
    GstFilter1, gst_filter1, GST_TYPE_BASE_TRANSFORM,
    GST_DEBUG_CATEGORY_INIT(gst_filter1_debug_category, "filter1", 0,
                            "debug category for filter1 element"));

static void gst_filter1_class_init(GstFilter1Class *klass) {
  ...
  base_transform_class->transform_caps =
      GST_DEBUG_FUNCPTR(gst_filter1_transform_caps);
  base_transform_class->fixate_caps =
      GST_DEBUG_FUNCPTR(gst_filter1_fixate_caps);
  base_transform_class->accept_caps =
      GST_DEBUG_FUNCPTR(gst_filter1_accept_caps);
  ...
  base_transform_class->decide_allocation =
      GST_DEBUG_FUNCPTR(gst_filter1_decide_allocation);
  base_transform_class->filter_meta =
      GST_DEBUG_FUNCPTR(gst_filter1_filter_meta);
  base_transform_class->propose_allocation =
      GST_DEBUG_FUNCPTR(gst_filter1_propose_allocation);
  base_transform_class->transform_size =
      GST_DEBUG_FUNCPTR(gst_filter1_transform_size);
  base_transform_class->get_unit_size =
      GST_DEBUG_FUNCPTR(gst_filter1_get_unit_size);
  ...
  base_transform_class->prepare_output_buffer =
      GST_DEBUG_FUNCPTR(gst_filter1_prepare_output_buffer);
  base_transform_class->copy_metadata =
      GST_DEBUG_FUNCPTR(gst_filter1_copy_metadata);
  base_transform_class->transform_meta =
      GST_DEBUG_FUNCPTR(gst_filter1_transform_meta);
  base_transform_class->before_transform =
      GST_DEBUG_FUNCPTR(gst_filter1_before_transform);
  base_transform_class->transform = GST_DEBUG_FUNCPTR(gst_filter1_transform);
  base_transform_class->transform_ip =
      GST_DEBUG_FUNCPTR(gst_filter1_transform_ip);
}

static void gst_filter1_init(GstFilter1 *filter1) {}

void gst_filter1_set_property(GObject *object, guint property_id,
                              const GValue *value, GParamSpec *pspec) {
...
void gst_filter1_get_property(GObject *object, guint property_id, GValue *value,
                              GParamSpec *pspec) {
  GstFilter1 *filter1 = GST_FILTER1(object);
...
void gst_filter1_dispose(GObject *object) {
  GstFilter1 *filter1 = GST_FILTER1(object);
...
void gst_filter1_finalize(GObject *object) {
  GstFilter1 *filter1 = GST_FILTER1(object);
...
static gboolean gst_filter1_sink_event(GstBaseTransform *trans,
  ...
  return GST_BASE_TRANSFORM_CLASS(gst_filter1_parent_class)
