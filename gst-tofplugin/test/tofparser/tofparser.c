...
StreamHeader aheader = {
  .container_header_size = 1024,
...
void setup_data(void) {
  ...
  sf_payload_size = aheader.frame_width *
...
GST_START_TEST (test_frame_meta)
{
  ...
  for (int df = 0; df < aheader.num_frames; df++) {
    ...
    fail_unless(outbuf_size == expected_size, "frame received has incorrect size");
    meta_count = 0;
    state = NULL;
