...
static std::map<uint32_t, ImGuiContext*> windowIdContextMap;
...
uint32_t GetWindowIdFromEvent(const SDL_Event* event) {
  switch (event->type) {
...
bool ImGui_ImplSDL2_ProcessEvent(const SDL_Event* event) {
  uint32_t windowId = GetWindowIdFromEvent(event);
  if (windowId != 0) {
    return ImGui_ImplSDL2_ProcessWindowEvent(event, windowId);
  }
  ...
  // so change of DPI/Scaling are not reflected in this event. (SDL3 has it)
    ...
    for (auto pair : windowIdContextMap) {
      ImGuiContext* ctx = pair.second;
...
bool ImGui_ImplSDL2_ProcessWindowEvent(const SDL_Event* event,
                                       uint32_t windowId) {
  ImGuiContext* ctx = windowIdContextMap[windowId];
  ImGui::SetCurrentContext(ctx);
  ImGuiIO& io = ImGui::GetIO();
  ImGui_ImplSDL2_Data* bd = ImGui_ImplSDL2_GetBackendData();
...
static bool ImGui_ImplSDL2_Init(SDL_Window* window, SDL_Renderer* renderer,
  ...
  windowIdContextMap[SDL_GetWindowID(window)] = ImGui::GetCurrentContext();
  return true;
}
...
static void ImGui_ImplSDL2_CreateWindow(ImGuiViewport* viewport) {
  ...
  windowIdContextMap[SDL_GetWindowID(vd->Window)] = ImGui::GetCurrentContext();
}
