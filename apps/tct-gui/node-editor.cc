...
void NodeEditor::ImGuiDraw() {
  ...
  // The input widgets also require you to manually disable the editor shortcuts
  // so the view doesn't fly around. (note that this is a per-frame setting, so
  // it disables it for all text boxes.  I left it here so you could find it!)
  ed::EnableShortcuts(!io.WantTextInput);
  // The input widgets require some guidance on their widths, or else they're
  // very large. (note matching pop at the end).
  ...
  // =======================================================================================================
  // TreeNodes and Headers streatch to the entire remaining work area. To put
  // them in nodes what we need to do is to tell ImGui out work area is shorter.
  // We can achieve that right now only by using columns API.
  //
  ...
  // Tree column startup
  // ------------------------------------------------------------------- Push
  // dummy widget to extend node size. Columns do not do that.
  ...
  // =====================================================================================
  // Tooltips, combo-boxes, drop-down menus need to use a work-around to place
  // the "overlay window" in the canvas. To do this, we must defer the popup
  // calls until after we're done drawing the node material.
  //
  ...
  if (ImGui::Button(popup_text)) {
    do_popup = true;  // Instead of saying OpenPopup() here, we set this bool,
  ...
  // This entire section needs to be bounded by Suspend/Resume!  These calls pop
  // us out of "node canvas coordinates" and draw the popups in a reasonable
  // screen location.
  ed::Suspend();
  // There is some stately stuff happening here.  You call "open popup" exactly
  // once, and this causes it to stick open for many frames until the user makes
  // a selection in the popup, or clicks off to dismiss. More importantly, this
  // is done inside Suspend(), so it loads the popup with the correct screen
  // coordinates!
  if (do_popup) {
    ImGui::OpenPopup("popup_button");  // Cause openpopup to stick open.
    do_popup = false;  // disable bool so that if we click off the popup, it
                       // doesn't open the next frame.
  }
  ...
  if (ImGui::BeginPopup("popup_button")) {
    ...
    if (ImGui::Button("Option 1")) {
      ...
      ImGui::CloseCurrentPopup();  // These calls revoke the popup open state,
                                   // which was set by OpenPopup above.
    }
    ...
    ImGui::EndPopup();  // Note this does not do anything to the popup
                        // open/close state. It just terminates the content
                        // declaration.
  }
  ...
  // Plot Widgets
  // =========================================================================================
  // Note: most of these plots can't be used in nodes missing, because they
  // spawn tooltips automatically, so we can't trap them in our deferred pop-up
  // mechanism.  This causes them to fly into a random screen location.
  ...
  // This was coppied from BasicInteration.cpp. See that file for commented
  // code.
  ...
  if (ed::BeginCreate()) {
    ...
    if (ed::QueryNewLink(&inputPinId, &outputPinId)) {
      if (inputPinId && outputPinId) {
        if (ed::AcceptNewItem()) {
          m_Links.push_back(
              {ed::LinkId(m_NextLinkId++), inputPinId, outputPinId});
          ed::Link(m_Links.back().Id, m_Links.back().InputId,
                   m_Links.back().OutputId);
          std::cout << "New link created: " << (int)inputPinId.Get() << ", "
                    << (int)outputPinId.Get() << "\n";
        }
      }
    }
